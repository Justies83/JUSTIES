...
- **독자가 이 주제를 찾아본 이유부터 답한다.** 정의와 배경을 길게 깔지
  않는다. 무슨 일이 있었는지, 나에게 어떤 영향이 있는지, 그래서 무엇을
  확인하거나 하면 되는지를 앞에 둔다.
- **처음 접한 사람이 읽어도 막히지 않게 쓴다.** 전문 용어·제도 이름·약어는
  처음 나올 때 한 구절로 풀어 준다. 다만 사전식 정의를 늘어놓는 글이 되면
  안 된다 — 그 사실이 독자에게 무슨 뜻인지까지 붙인다.
- **구체적으로 쓴다.** 언제·얼마·누가·어떤 절차인지 적는다. 해당되는 사람과
  아닌 사람, 예외, 헷갈리기 쉬운 지점을 구분해 준다. 다만 **겪지 않은 개인
  경험을 지어내지 않는다** — "제가 직접 해보니", "저도 그랬습니다" 같은
  1인칭 체험담은 쓰지 않는다.
- **단정하지 않는다.** "100% 보장", "무조건", "반드시" 같은 확언과 수익·효과를
  약속하는 표현을 쓰지 않는다. 조건과 한계, 아직 정해지지 않은 부분을 함께
  적는 편이 독자에게 더 도움이 된다.
- **주제어를 기계적으로 반복하지 않는다.** 같은 낱말을 억지로 여러 번 끼워
  넣지 않고 문맥에 맞게 쓴다. 검색을 의식해 쓴 문장은 읽는 사람이 바로
  알아챈다.
